...
void Actuators::configurePins() {
  ...
  // Port 1
  outputs = MOTOR_LEFT_PIN | MOTOR_RIGHT_PIN | MOTOR_SLOW_PIN | MOTOR_STOP_PIN |
            LAMP_RED_PIN | LAMP_YELLOW_PIN | LAMP_GREEN_PIN | SWITCH_PIN;
...
void Actuators::setRedBlinking(bool on, bool fast) {
  if (redBlinking) {
    redBlinking = false;
    if (th_RedBlinking.joinable()) th_RedBlinking.join();
  }
